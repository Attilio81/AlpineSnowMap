...
#### Agente Web
- **Fonte:** DuckDuckGo/Google (via `ddgs`, gratuito, no API key)
...
| Ricerca web | DuckDuckGo/Google via ddgs (gratuito, no API key) |
...
| `GET` | `/api/peaks` | Cime vicine (Overpass) |
| `GET` | `/api/snow` | Copertura neve Sentinel-2 |
| `GET` | `/api/slope` | Pendenza DEM |
| `GET` | `/mcp/sse` | MCP SSE endpoint per Claude Desktop |
